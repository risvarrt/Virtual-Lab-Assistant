...
class CollegeRegister extends Component {
  ...
  onUserRegister = async(e) => {
      e.preventDefault();
    try {
      var loginRes = await Axios.post("http://localhost:5000/users/loginStudent",{ 
      college_id: this.state.college_id,
      email: this.state.email,
      password: this.state.password 
    });
      sessionStorage.setItem("collegeId", this.state.college_id);
      sessionStorage.setItem("studentId", loginRes.data.reg_no);
      sessionStorage.setItem("name", loginRes.data.name);
      sessionStorage.setItem("role", "student");
      console.log(loginRes.data);
      this.props.history.push("/user/student/dashboard");
    } catch (err) {
          ...
          <Card className="auth-card">
            <div className="position-relative image-side ">
              ...
              <p className="white mb-0">
                ...
                <NavLink to={`/user/colleges/studentRegister`} className="white">
                  register
                </NavLink>
                .
              </p>
